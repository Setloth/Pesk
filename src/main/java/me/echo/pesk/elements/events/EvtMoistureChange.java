...
public class EvtMoistureChange extends SkriptEvent {
    ...
    static {
        if (Skript.classExists("org.bukkit.event.block.MoistureChangeEvent")) {
            Skript.registerEvent("Moisture Change", EvtMoistureChange.class, MoistureChangeEvent.class, "[block] moisture [level] chang(e|ing) [(of|for) %-itemtypes%]");
        }
    }
    ...
    public boolean check(Event e) {
        if (types != null) {
            for (ItemType type : types.getAll()) {
                if (new ItemType(((MoistureChangeEvent) e).getBlock()).equals(type))
                    return true;
            }
            return false;
        }
