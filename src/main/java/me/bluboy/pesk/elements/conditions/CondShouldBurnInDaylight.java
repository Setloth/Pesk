package me.bluboy.pesk.elements.conditions;

import ch.njol.skript.Skript;
import ch.njol.skript.conditions.base.PropertyCondition;
import org.bukkit.entity.LivingEntity;
import org.bukkit.entity.Zombie;

public class CondShouldBurnInDaylight extends PropertyCondition<LivingEntity> {

    static {
        if (Skript.classExists("org.bukkit.entity.Zombie")) {
            register(CondShouldBurnInDaylight.class, PropertyType.CAN, "burn [in] day[( |-|)light]", "livingentities");
        }
    }

    @Override
    public boolean check(final LivingEntity e) {
        if (!(e instanceof Zombie)) return false;
        return ((Zombie) e).shouldBurnInDay();
    }

    @Override
    protected String getPropertyName() {
        return "burn in daylight";
    }
}
